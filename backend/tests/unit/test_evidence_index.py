...
from experiments.evidence_index import _resolve_unicode_path, atomic_write_json, build_indexes, load_registry, normalize_text, validate_index, validate_verified_rule_mappings
...
def test_local_official_pdfs_build_deterministically(tmp_path):
    ...
    assert sum(row["status"] == "verified" for row in audit["rules"].values()) == 3
