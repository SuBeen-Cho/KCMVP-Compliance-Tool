...
서브 모듈:
  prompt_templates   — 룰별 L3/DOC 프롬프트 템플릿 + 고위험 규칙 목록
  gemini_client      — Gemini/OpenAI/Local LLM 호출 + JSON 파싱 + 재시도
  candidate_selector — L3 대상 선정 (버킷 방식 + L1.5 이름 필터)
  code_context       — 코드 절삭 (symbol_graph 연동)
  prompt_builder     — 단일/배치/재판정 프롬프트 빌더 + 구조화 증거
  l3_judge           — 메인 L3 실행 (run_l3_contextualizer)
  doc_judge          — DOC L3 판정 (run_doc_l3_contextualizer)
